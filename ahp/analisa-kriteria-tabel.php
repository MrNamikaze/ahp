	...
	<div class="col-xs-12 col-sm-12 col-md-12">
      ...
      <tbody>
					...
					<?php $stmt5 = $bobotObj->readAll2(); while ($row = $stmt5->fetch(PDO::FETCH_ASSOC)): ?>
						<th>
							<?php
								...
								$jumlah1[] = number_format($bobotObj->nak, 4, '.', ',');
		...
		<table width="100%" class="table table-striped table-bordered">
			...
			<tbody>
				<?php $total=0; $asd = 0; $bobots1z = $bobotObj->readAll2(); while ($row1 = $bobots1z->fetch(PDO::FETCH_ASSOC)): ?>
					<tr>
						<th class="active"><?=$row1["nama_kriteria"]?></th>
						<th class="info"><?=$jumlah1[$asd]?></th>
						<th class="success"><?=number_format($row1["bobot_kriteria"], 4, '.', ',');?></th>
						<?php $jumlah = $jumlah1[$asd] * $row1["bobot_kriteria"]; ?>
						<th class="warning"><?=number_format($jumlah, 4, '.', ',');?></th>
						<?php $total += $jumlah; $asd++?>
					</tr>
				<?php endwhile; ?>
			</tbody>
			<tfoot>
